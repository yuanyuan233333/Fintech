models_perf = function(formula, train, test, model){
  ...
  if (model == "glm" || model == "lda" || model == "rpart"){
    for (i in seq(along = train)){
      ...
      INDEX=test[[i]]$status==1
      INDEX_N=test[[i]]$status==0
      #Library (PRROC)
      #fit_pr[[i]]=pr.curve(scores.class0 = fit_score[[i]][INDEX,2], scores.class1 = fit_score[[i]][INDEX_N,2])[2]
  
      #fit_score[[i]][1,INDEX]=1
      #fit_score[[i]][2,INDEX]=0
